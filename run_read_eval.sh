...
for run in `seq 1 $nrun`; do
    for n_routine in $N_ROUTINES; do
        _size in $PAYLOAD_SIZES; do
            ...
            sudo ./proxy-tun read $payload_size $n_routine > $res_path/read_$exp_code.csv &
            sudo ./setup_tun.sh

            nping --udp 192.168.0.1 -c 0 --data-length $payload_size -e tun0 --rate 10000000 -N 2> /dev/null 1> /dev/null &

            sar -A -o $res_path/read_sar_$exp_code.data 1 $DURATION > /dev/null &
